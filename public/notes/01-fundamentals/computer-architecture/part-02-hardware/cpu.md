...
A **CPU** (central processing unit) executes instructions, performs arithmetic, and coordinates work on a chip built from **transistors** (see [[Electronic_Circuits]]). You find CPUs in phones, laptops, servers, cars, ATMs, and many embedded devices.
...
This hierarchy is why “disk-bound” workloads behave differently from “CPU-bound” ones. More detail lives in [[Memory_Storage_Filesystems_and_Paths]].
...
- [[Electronic_Circuits]]
- [[SoC_and_GPU]] — graphics and AI throughput often come from **GPUs** or **NPUs**, not the CPU alone
- [[Boot_Sequence]] — early instructions the CPU runs come from firmware and the bootloader
- [[Kernel]] — the OS kernel schedules **which** code runs on the CPU and **how** memory is virtualized
